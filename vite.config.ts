import { defineConfig } from 'vite';
import react from '@vitejs/plugin-react';
import { VitePWA } from 'vite-plugin-pwa';

// https://vitejs.dev/config/
export default defineConfig({
  base: '/toptanci3/', // GitHub Pages için gerekli olan ana dizin ayarı
  plugins: [
    react( ),
    VitePWA({
      registerType: 'autoUpdate',
      workbox: {
        globPatterns: ['**/*.{js,css,html,ico,png,svg,json,txt,woff2}'],
        runtimeCaching: [
          {
            urlPattern: /^https:\/\/fonts\.googleapis\.com\/.*/i,
            handler: 'CacheFirst',
            options: {
              cacheName: 'google-fonts-cache',
              expiration: {
                maxEntries: 10,
                maxAgeSeconds: 60 * 60 * 24 * 365
              }
            }
          }
        ]
      },
      includeAssets: ['favicon.ico', 'apple-touch-icon.png'],
      manifest: {
        name: 'Toptancı Takip - Kolay ve Okunabilir Toptancı Yönetimi',
        short_name: 'Toptancı Takip',
        description: 'Toptancı işlemlerinizi kolay ve güvenli bir şekilde takip edin.',
        theme_color: '#2563eb',
        background_color: '#ffffff',
        display: 'standalone',
        scope: '/toptanci3/', // Alt dizine uygun hale getirildi
        start_url: '/toptanci3/', // Alt dizine uygun hale getirildi
        orientation: 'portrait-primary',
        lang: 'tr',
        icons: [
          {
            src: 'icon-192x192.png', // Baştaki / kaldırıldı (relative path )
            sizes: '192x192',
            type: 'image/png',
            purpose: 'any maskable'
          },
          {
            src: 'icon-512x512.png', // Baştaki / kaldırıldı (relative path)
            sizes: '512x512',
            type: 'image/png',
            purpose: 'any maskable'
          }
        ]
      },
      devOptions: {
        enabled: true
      }
    })
  ],
  optimizeDeps: {
    exclude: ['lucide-react'],
  },
  build: {
    chunkSizeWarningLimit: 1000,
    rollupOptions: {
      output: {
        manualChunks: {
          vendor: ['react', 'react-dom'],
          firebase: ['firebase/app', 'firebase/auth', 'firebase/database'],
          ui: ['lucide-react']
        }
      }
    }
  }
});
